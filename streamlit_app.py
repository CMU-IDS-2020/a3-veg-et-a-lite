...
def asset_dropdown():
    ...
    asset_name = st.selectbox('Choose an asset', [*asset_map.keys()])
...
def metrics_dropdown(asset_metrics):
    ...
    metric_name = st.selectbox('Choose a metric', metric_choices)
    metric_id = name_to_id_map[metric_name]
    metric_description = id_to_info_map[metric_id][1]

    with st.beta_expander("See explanations for metric"):
